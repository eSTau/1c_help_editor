...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТекстРедактирования") Тогда
		ТекстСообщения = Параметры.ТекстРедактирования;
	КонецЕсли; 
	
	Если Параметры.Свойство("ИдентификаторТекста") И Не Параметры.ИдентификаторТекста = "" Тогда
		ИдентификаторСообщения = Параметры.ИдентификаторТекста;
	    ФормаОткрытаВРежимеРедактированияСообщения = Истина;
	КонецЕсли;
	
	Если Параметры.Свойство("ТипСообщения") Тогда
		ТипСообщения = Параметры.ТипСообщения;
	Иначе
		//ТипСообщения = Перечисления.рдс_ТипыСообщенийЧата.Комментарий;
	КонецЕсли; 
	...
	Если Параметры.Свойство("РежимПросмотра") Тогда
		...
		Если Параметры.Свойство("РежимПросмотраПолнойПереписки") Тогда
			ПоказатьПерепискуДокумента(Параметры.ФильтрыЧата);
		Иначе
			
			ЗаполнитьСписокФайловДокумента();
			//ПоказатьТекстВФорматеHtml(ТекстСообщения, мРасшифровкиСсылок);	
			
		КонецЕсли; 
	Иначе
		
		//Конфигурация = Справочники.Конфигурации.НайтиПоНаименованию("РДС ПРОБА");
		//ПоказатьДеревоМетаданныхКонфигурации(Конфигурация);
		   //ЗаполнитьСписокФайловДокумента();
...
Процедура ПриОткрытии(Отказ)
	...
	Если РежимПросмотра Тогда
		УстановитьРежимРаботыСРедактором(Ложь, Ложь);		
	КонецЕсли; 
...
КонецПроцедуры // УстановитьПервоначальныеНастройки()
...
КонецПроцедуры // СохранитьИконкиСправки()
